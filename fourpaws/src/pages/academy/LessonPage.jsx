...
import { ChevronLeft, ChevronRight, CheckCircle, Play, Download, Clock, BookOpen, Flame, Trophy } from 'lucide-react'
...
export default function LessonPage() {
  ...
  const { checkMilestone, dogProfile }    = useAI()
  ...
  const dogName       = dogProfile?.name || state.dogProfile?.name || null
  ...
  const handleComplete = () => {
    ...
    if (!isAlreadyDone) {
      ...
      else notify(dogName ? `Lesson complete! Great work with ${dogName}.` : 'Lesson complete! Excellent work.', 'success')
    }
    setCompleted(true)
    setTimeout(() => {
      if (nextLesson) navigate(`/academy/course/${courseId}/lesson/${nextLesson.id}`)
    ...
    <div className="min-h-screen p-6 lg:p-10 max-w-4xl mx-auto">
      ...
      <AnimatePresence>
        {milestone && (
          ...
          >
            <div className="glass-card p-10 text-center max-w-sm" style={{ border: '1px solid rgba(201,168,76,0.5)' }}>
              <motion.div
                animate={{ rotate: [0, -10, 10, -5, 5, 0] }}
                transition={{ duration: 0.6 }}
                className="text-6xl mb-4"
              >🏆</motion.div>
              <div className="section-label mb-2">Milestone Unlocked</div>
              <h2 className="luxury-heading text-2xl mb-2">{milestone.label}</h2>
              {dogName && (
                <p className="font-sans text-sm font-light text-silver-400">
                  {dogName}'s journey reaches a new height.
                </p>
              )}
            </div>
          </motion.div>
      ...
      <FadeIn className="mb-6">
        ...
        <div className="flex items-center gap-4 mt-3 flex-wrap">
          <div className="flex items-center gap-1 text-silver-500"><Clock size={12} /><span className="font-sans text-xs">{lesson.duration}</span></div>
          <div className="flex items-center gap-1 text-silver-500"><BookOpen size={12} /><span className="font-sans text-xs capitalize">{lesson.type}</span></div>
          <div className="font-sans text-xs text-silver-600">Lesson {currentIndex + 1} of {allLessons.length}</div>
          {isAlreadyDone && (
            <div className="flex items-center gap-1 text-gold-500"><CheckCircle size={12} /><span className="font-sans text-xs">Complete</span></div>
          )}
        </div>
      </FadeIn>
      ...
      {/* Video placeholder */}
      <FadeIn className="mb-8">
        <div className="glass-card gold-border aspect-video relative overflow-hidden cursor-pointer group">
          <div className="absolute inset-0 flex flex-col items-center justify-center"
            ...
            {dogName && (
              <div className="font-sans text-[10px] text-gold-600 mt-3 tracking-widest uppercase">
                Personalised for {dogName}
              </div>
            )}
          </div>
          <div className="absolute top-0 right-0 w-40 h-40" style={{ background: 'radial-gradient(circle at 100% 0%, rgba(201,168,76,0.08) 0%, transparent 70%)' }} />
        </div>
      </FadeIn>
      ...
      <FadeIn className="glass-card p-8 mb-6" style={{ border: '1px solid rgba(255,255,255,0.05)' }}>
        ...
        <div className="space-y-4 font-sans text-sm font-light text-silver-400 leading-relaxed">
          ...
          <p>
            ...
            have a clear action plan for your next session, and feel confident applying these principles
            {dogName ? ` with ${dogName}` : ' with your own dog'}.
          </p>
          <p>
      ...
      <FadeIn className="glass-card p-8 mb-6" style={{ border: '1px solid rgba(255,255,255,0.05)' }}>
        ...
        <div className="space-y-3">
          {[
            ...
            `How to read ${dogName || 'your dog'}'s responses and adjust accordingly`,
            'Building this skill into your daily routine seamlessly',
          ].map((t, i) => (
            <motion.div key={t} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 + i * 0.07 }}
      ...
      {/* Download */}
      <FadeIn className="mb-8">
        <motion.button whileHover={{ borderColor: 'rgba(201,168,76,0.5)' }}
      ...
      <FadeIn className="flex items-center gap-4 flex-wrap">
        {prevLesson && (
          <button onClick={() => navigate(`/academy/course/${courseId}/lesson/${prevLesson.id}`)}
        ...
        <motion.button onClick={handleComplete} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
          className="btn-gold flex items-center gap-2 text-xs flex-1 sm:flex-none justify-center">
